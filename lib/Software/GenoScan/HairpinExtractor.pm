	...
	foreach my $file (@foldFiles){
		...
		for(my $index = 0; $index < scalar(@fold); $index++){
			if($fold[$index] =~ /^>/){
				...
				#If window contains no hairpin loop, ignore
				if($numHairpinLoops == 0){
					next;
				}
			
				#Iterate over all hairpin loops in window
				...
				foreach my $index (0 .. scalar(@hairpinLoopStartPos) - 1){
					...
					my @hairpinArray = ("HP:$SPECIES_CODE:$chr:$lociStart-$lociEnd", $hairpinSeq, $hairpinStr, "$chr:$lociStart-$lociEnd", $winID, $hairpinSeqLen, $cgComp, $trSig, $stemLen, $loopLen, $stembpProp, $upStrNor, $upLenNor, $seqLenPass, $cgCompPass, $trSigPass, $stemLenPass, $loopLenPass, $stemBpPrPass, $upStrNorPass, $upLenNorPass);
					if($seqLenPass && $cgCompPass && $trSigPass && $stemLenPass && $loopLenPass && $stemBpPrPass && $upStrNorPass && $upLenNorPass){
						$numExtracted++;
						print(EXTRACTED ">HP$numExtracted species $SPECIES_CODE loci $chr:$lociStart-$lociEnd\n$hairpinSeq\n\n");
					}
					$numSTL++;
				}
			}
