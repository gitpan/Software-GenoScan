...
sub annotateHSS($ $){
	...
	foreach my $mir (@mirnas){
		...
		for(my $pos = 3; $pos < scalar(@{$visualMatrix[0]}) - 2; $pos++){
			if($visualMatrix[0][$pos] =~ m/[AUCG]/ || $visualMatrix[1][$pos] =~ m/[AUCG]/){
				$nucIndex++;
			}
		...
		for(my $pos = 3; $pos < scalar(@{$visualMatrix[0]}) - 2; $pos++){
			...
			if($tailPassed && $nucIndex < $hairpinLoopStart){		#Stem passed the tail
				if($visualMatrix[2][$pos] eq "|"){						#Basepair or wobble
					if(($visualMatrix[1][$pos] eq "G" && $visualMatrix[3][$pos] eq "U") || ($visualMatrix[1][$pos] eq "U" && $visualMatrix[3][$pos] eq "G")){
					...
					my $fiveNuc = () = $fivePrime =~ m/[AUGC]/g;
					my $fiveNucComp = 0;
					if($fivePrime =~ m/^[AUGC]/){
						$fiveNucComp = 1;
					}
					if($fivePrime =~ m/^[AUGC]+$/ && $threePrime =~ m/^[AUGC]+$/){
						$mismatchType = "sym";
					}
					elsif(($fivePrime =~ m/^[AUGC]+$/ && $threePrime =~ m/[AUGC]/ && $threePrime =~ m/-/) || ($fivePrime =~ m/[AUGC]/ && $fivePrime =~ m/-/ && $threePrime =~ m/^[AUGC]+$/)){
						$mismatchType = "asym";
					}
					elsif(($fivePrime =~ m/^[AUGC]+$/ && $threePrime =~ m/^-+$/) || ($fivePrime =~ m/^-+$/ && $threePrime =~ m/^[AUGC]+$/)){
						$mismatchType = "bulge";
					}
